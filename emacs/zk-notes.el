  ...
  (save-excursion
    ...
    (while (re-search-forward image-regex nil t)
      ...
      (let ((ov (make-overlay  beg end)))
        (overlay-put ov 'display image) ; 'after-string "t")
...
(defun underline-region (start end)
  "Underline the region between START and END."
  "this works in a vanilla emacs settings "
  (interactive "r")
  (put-text-property start end 'face '(:underline t)))


(defun zk--button-handler (button)
  (zk-open-link-at-point))
...
(defun zk-colorize-links()
  ...
  (save-excursion
    ...
    (while (re-search-forward links-regex nil t)
      ...
      (let ((button (make-button begin end)))
        (overlay-put button 'action 'zk--button-handler))
      ;; (make-button begin end  'my-func)
      ;; (let ((ov (make-overlay begin end)))
      ;;   (overlay-put ov 'font-lock-face
      ;;                '((:foreground "LightSkyBlue")
      ;;                  (:underline t)))
      ;;   (overlay-put ov 'mouse-face 'highlight)
      ;;   ;; (overlay-put ov 'on-click 'my-func)
      ;;   )
      )))
...
(global-set-key (kbd "C-c j j") #'zk-open-link-at-point)
(global-set-key (kbd "M-o") #'zk-open-link-at-point)
;; (define-key "markdown" (kbd "M-o") #'zk-open-link-at-point)
;; (local-set-key 'markdown (kbd "M-o") #'zk-open-link-at-point)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
 ...
 (add-hook 'markdown-mode-hook (lambda ()
                                  (zk-colorize-links)
                                  (zk-render-image-links)
                                  (local-set-key (kbd "M-o") #'zk-open-link-at-point)
                                  ))
  (provide 'zk-notes)
